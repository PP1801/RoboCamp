...
void setup() {
  ...
  if (autoload > 0)
  {
    if (bitRead(autoload, 0))   // autoload LED stage
    {
      stage = EEPROM.read(1);
      set_rgb(stage);
      Serial.print("Setting stage to: "); Serial.println(stage);    
    }
...
void do_command(String command)
{
  ...
  else if (command == "pass") // change Wi-Fi password
  {
  ...
  else if (command == "savew")// save Wi-Fi data to EEPROM
  {
    ...
    wifiName[15] = '\0';
    Serial.print("seeWiFi:"); Serial.println(wifiName);
    ...
    wifiPass[15] = '\0';
    Serial.print("seePass:"); Serial.println(wifiPass);
    ...
    Serial.print("seePort:"); Serial.println(ipPort);
...
void load_wifi()    // load Wi-Fi name and password from EEPROM
{
  ...
  wifiName[15] = '\0';
  //Serial.println("");
  Serial.print("eeWiFi:"); Serial.println(wifiName);
  ...
  wifiPass[15] = '\0';
  //Serial.println("");
  Serial.print("eePass:"); Serial.println(wifiPass);
...
void load_ip()      // load IP address and port from EEPROM
{
  ...
  Serial.print("eePort:"); Serial.println(ipPort);
